...
public class FilmArchiveGrader {
	...
	private final static Class<?>[] STRING_INT_TYPE_LIST = { String.class,
			int.class };
	...
	private static void checkFilmArchiveStructure() {
		...
		try {
			filmArchiveClass.getDeclaredMethod("getSorted", EMPTY_TYPE_LIST); // getSorted()
																				// ok?
		} catch (NoSuchMethodException x) {
			abortZeroPoints("No getSorted() method in FilmArchive");
		}
		try {
			Method addMeth = filmArchiveClass.getDeclaredMethod("add",
					MOVIE_TYPE_LIST); // add() ok?
	...
	private static void checkMovieStructure() {
		...
		try {
			Method cmpMeth = movieClass.getDeclaredMethod("compareTo",
					MOVIE_TYPE_LIST); // compareTo()
		...
		try // getTestMovies()
		{
			Method getTestMoviesMeth = movieClass.getDeclaredMethod(
					"getTestMovies", EMPTY_TYPE_LIST);
		...
		try // getTitle()
		{
			Method getTitleMeth = movieClass.getDeclaredMethod("getTitle",
					EMPTY_TYPE_LIST);
		...
		try // getYear()
		{
			Method getYearMeth = movieClass.getDeclaredMethod("getYear",
					EMPTY_TYPE_LIST);
	...
	private static void checkImplementorsStructure() {
		String[] classNames = { "ListFilmArchive", "HashFilmArchive",
				"TreeFilmArchive" };
		for (String className : classNames) {
			try {
				...
				if (!implementsOk) // implements Comparable?
					abortZeroPoints(className
							+ " class doesn't declare that it implements FilmArchive");
	...
	private static void checkMovieFunctionality() {
		...
		for (Movie m : movies) {
			...
			if (m.hashCode() != goldenHashCode) {
				String err = "Bad hashCode for " + movieToString(m)
						+ ": expected " + goldenHashCode + ", saw "
						+ m.hashCode();
		...
		for (int i = 0; i < these.length; i++) {
			...
			if (Math.signum(cmp) != expects[i]) {
				...
				s += " when comparing {" + movieToString(these[i]) + "} to {"
						+ movieToString(those[i]) + "}";
				s += " ... expected ";
				if (expects[i] == 0)
					s += "0";
				else if (expects[i] < 0)
		...
		if (testMovies.length != 10) {
			String s = "Movie.getTestMovies() returned an array of length "
					+ testMovies.length + ", expected 10";
		...
		if (!m0.getTitle().equals(m1.getTitle()))
			reasonToDeductions.put(
					"Items 0 and 1 in test movies array don't have same title",
					10);
		if (m0.getYear() == m1.getYear())
			reasonToDeductions
					.put("Items 0 and 1 in test movies array don't have different years",
							10);
		Movie m2 = testMovies[2];
		Movie m3 = testMovies[3];
		if (m2.getTitle().equals(m3.getTitle()))
			reasonToDeductions
					.put("Items 2 and 3 in test movies array don't have different titles",
							10);
		if (m2.getYear() != m3.getYear())
			reasonToDeductions.put(
					"Items 2 and 3 in test movies array don't have same years",
					10);
	}
